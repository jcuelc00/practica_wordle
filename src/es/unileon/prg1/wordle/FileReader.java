	...
	public String read() /* throws WordleException */ {
		...
		try {
			...
			System.err.println(new IOException(
					"FileReader:read - Un error de entrada o salida ha imposibilitado que se pudiera abrir el fichero "
							+ this.fileName));

			logger.warn("FileReader:read - No se ha podido leer el fichero " + this.fileName);
			/*
			 * throw new
			 * WordleException("Un error de entrada o salida ha imposibilitado que se pudiera abrir el fichero "
			 * + this.fileName);
			 */
		} finally {
			/*
			...
			try {
				...
				System.err.println(new IOException(
						"FileReader:read - Un error de entrada o salida ha imposibilitado que se pudiera cerrar el fichero "
								+ this.fileName));

				logger.warn("FileReader:read - El fichero " + this.fileName + " no se ha cerrado correctamente");
				/*
				 * throw new
				 * WordleException("Un error de entrada o salida ha imposibilitado que se pudiera cerrar el fichero "
				 * + this.fileName);
				 */
			}
		}
